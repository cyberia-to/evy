...
impl EnableExtensions {
    ...
    pub(crate) fn add(&mut self, ext: ImplementedEnableExtension) {
        match ext {
            ImplementedEnableExtension::Subgroups => {} // accepted, no tracking needed
        }
    }
    ...
    pub(crate) const fn contains(&self, ext: ImplementedEnableExtension) -> bool {
        match ext {
            ImplementedEnableExtension::Subgroups => true, // always enabled when requested
        }
    }
}
...
pub enum ImplementedEnableExtension {
    /// Enables subgroup operations (subgroupAdd, etc.)
    Subgroups,
}
